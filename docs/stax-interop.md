...
- An **anchored band** (one that has a non-empty `RatingRubric` description) is
  named `"{band}-{desc}"`, for example `1-poor` or `5-great`.
- Every **other band** is named `"score-{band}"`, for example `score-2`,
  `score-3`, `score-4`.

When a template declares no `RatingRubric`, all bands fall back to `score-{band}`
across the `rubricDetail.scale` range (default 1 to 5).
...
- **`model_id` is a user-supplied deployment binding, never migrated.** Mizan
  ...
  POST.
- **Stax marks `model_id` `@NotNull`, not `@NotBlank`.** An empty string passes
  Stax's server-side validation, so an artifact with an unfilled `model_id` can
  be accepted yet is not actually usable. **Do not POST an unfilled artifact;
  fill `--model-id` first.**
  <!-- TODO(em-ruling): finalize the exact unset-model_id emit behavior once the owner rules. Current build emits "model_id": "" plus a stderr warning; the open decision is emit-empty-with-warning vs omit-the-field-to-fail-closed at Stax's @NotNull boundary. Fill this one sentence in when the ruling lands; the "always supply --model-id" guidance above holds either way. -->
